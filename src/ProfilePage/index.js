...
export default function SingleRecipePage () {
  ...
  const fetchSelected = useInfiniteQuery([selectedDataSet, username, searchParams.get('q')],
    ({ pageParam = Math.floor(Date.now() / 1000).toString() }) => api[apiFnName](username, searchParams.get('q') || '', pageParam),
    {
  ...
  const fetchProfile = useQuery(['profile', username, searchParams.get('q')],
    ({ pageParam = Math.floor(Date.now() / 1000).toString() }) => api.getProfile(username, searchParams.get('q') || '', pageParam),
    {
      getNextPageParam: (lastPage, pages) => lastPage.nextCursor,
      refetchOnMount: false
    })
  ...
  return (
    <Page sideSelected={selectedSidebarLink} header={`${username}'s Profile`}>
